      ...
      <section className="grid min-h-0 flex-1 gap-2 xl:grid-cols-[minmax(0,1.1fr)_minmax(360px,0.9fr)]">
        ...
        <Tabs value={safeActiveTab} onValueChange={(value) => setActiveTab(value as typeof safeActiveTab)} className="min-h-0 flex flex-col overflow-hidden">
          <TabsList className="grid h-auto w-full shrink-0 grid-cols-6 gap-1 rounded-xl border border-border/80 bg-card/92 p-1">
            ...
            <TabsTrigger value="lobbying" title={lobbyingUnlocked ? 'Lobbying unlocked' : 'Unlock with Regulatory Affairs in Research'} className={`gap-1.5 py-2 text-[10px] uppercase tracking-[0.12em] data-active:bg-primary data-active:text-primary-foreground ${!lobbyingUnlocked ? 'border-border/60 bg-[linear-gradient(180deg,rgba(50,50,50,0.55),rgba(28,28,28,0.55))] text-muted-foreground' : ''}`}><Landmark className="size-3.5" />Lobbying</TabsTrigger>

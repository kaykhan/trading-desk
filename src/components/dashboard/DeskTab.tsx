...
function UnitPanel({ config, incomeLabel, totalCost, nextCost, quantity, buyMode, disabled, unlocked, titleDescription, onBuy, onModeChange }: {
  ...
  const status = unlocked ? (disabled ? 'Need cash' : 'Ready') : 'Locked'
  const statusTone = unlocked ? (disabled ? 'default' : 'ready') : 'locked'
  const badges = [`${quantity} owned`, `${config.totalLabel} ${incomeLabel}`, ...(config.extraBadges ?? [])]
  ...
  return (
    <div className={!unlocked ? 'opacity-60' : undefined}>
      <PurchaseCard
        ...
        disabledReason={!unlocked ? config.lockedReason : disabled ? 'Not enough cash for current buy mode.' : undefined}
        badges={badges}
        onClick={onBuy}
...
export function DeskTab() {
  ...
  const humanTradingPowerUsage = useGameStore(selectors.humanTradingPowerUsage)
  ...
  const getUnitState = (unitId: UnitId) => {
    ...
    if (unitId === 'intern') {
      return { unlocked, disabled, totalCost, quantity, nextCost: nextInternCost, buyMode: internBuyMode, count: gameState.internCount, totalIncome: formatRate(internIncome), description: UNITS.intern.description }
    }
    ...
    if (unitId === 'propDesk') {
      return { unlocked, disabled, totalCost, quantity, nextCost: nextPropDeskCost, buyMode: propDeskBuyMode, count: gameState.propDeskCount, totalIncome: formatRate(propDeskIncome), description: UNITS.propDesk.description }
    }

    if (unitId === 'institutionalDesk') {
      return { unlocked, disabled, totalCost, quantity, nextCost: nextInstitutionalDeskCost, buyMode: institutionalDeskBuyMode, count: gameState.institutionalDeskCount, totalIncome: formatRate(institutionalDeskIncome), description: UNITS.institutionalDesk.description }
    }

    if (unitId === 'hedgeFund') {
      return { unlocked, disabled, totalCost, quantity, nextCost: nextHedgeFundCost, buyMode: hedgeFundBuyMode, count: gameState.hedgeFundCount, totalIncome: formatRate(hedgeFundIncome), description: UNITS.hedgeFund.description }
    }

    if (unitId === 'investmentFirm') {
      return { unlocked, disabled, totalCost, quantity, nextCost: nextInvestmentFirmCost, buyMode: investmentFirmBuyMode, count: gameState.investmentFirmCount, totalIncome: formatRate(investmentFirmIncome), description: UNITS.investmentFirm.description }
    }

    if (unitId === 'ruleBasedBot') {
      return { unlocked, disabled, totalCost, quantity, nextCost: nextRuleBasedBotCost, buyMode: ruleBasedBotBuyMode, count: gameState.ruleBasedBotCount, totalIncome: formatRate(ruleBasedBotIncome), description: UNITS.ruleBasedBot.description }
    }

    if (unitId === 'mlTradingBot') {
      return { unlocked, disabled, totalCost, quantity, nextCost: nextMlTradingBotCost, buyMode: mlTradingBotBuyMode, count: gameState.mlTradingBotCount, totalIncome: formatRate(mlTradingBotIncome), description: UNITS.mlTradingBot.description }
    }

    if (unitId === 'aiTradingBot') {
      return { unlocked, disabled, totalCost, quantity, nextCost: nextAiTradingBotCost, buyMode: aiTradingBotBuyMode, count: gameState.aiTradingBotCount, totalIncome: formatRate(aiTradingBotIncome), description: UNITS.aiTradingBot.description }
    }
    ...
    if (unitId === 'juniorPolitician') {
      return { unlocked, disabled, totalCost, quantity, nextCost: nextJuniorPoliticianCost, buyMode: juniorPoliticianBuyMode, count: gameState.juniorPoliticianCount, totalIncome: `${formatNumber(gameState.juniorPoliticianCount * 0.01, { decimalsBelowThreshold: 2 })} inf / sec`, description: UNITS.juniorPolitician.description }
    }
    ...
    <div className="h-full min-h-0 overflow-y-auto pr-1">
      <div className="space-y-2">
        <Tabs value={activeDeskView} onValueChange={(value) => setActiveDeskView(value as DeskViewId)} className="min-h-0 flex flex-col overflow-hidden">
          <TabsList className="grid h-auto w-full shrink-0 grid-cols-5 gap-1 rounded-xl border border-border/80 bg-card/92 p-1">
